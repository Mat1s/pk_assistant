...
class CarsImport
  ...
  def self.import
    ActiveRecord::Base.connection.insert("INSERT INTO type_cars (year, make, model) VALUES (1909, 'Ford', 'Model T'),
    ...
    (2013, 'Volvo', 'XC90') ");
    puts 'Cars imported'
    puts 'Thanks https://github.com/n8barr/automotive-model-year-data'
  end

  def self.import_from_json
    CARS_FROM_JSON.each do |elem|
      elem["models"].each do |model|
        next if TypeCar.where('make = ? and model = ?', elem['brand'], model).count > 0
        TypeCar.create(make: elem['brand'], model: model)
      end
    end
    TypeCar.where(year: nil).update(year: rand(2013..2017))
    puts 'Cars from json imported'
    puts 'Thanks https://github.com/FormidableLabs/radon-typeahead'
  end
end